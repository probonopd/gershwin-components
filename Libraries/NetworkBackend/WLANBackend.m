...
- (id)init
{
    ...
    if (self) {
        ...
        if (_isLinux) {
            _nmcli = [findTool(@"nmcli") retain];
        }
        if (_isBSD) {
            _wpaCli = [findTool(@"wpa_cli") retain];
            _ifconfig = [findTool(@"ifconfig") retain];
            _dhclient = [findTool(@"dhclient") retain];
            _sysctl = [findTool(@"sysctl") retain];
        }
    }
    return self;
}
...
- (BOOL)linuxIsEnabled
{
    NSLog(@"WLANBackend: linuxIsEnabled");
...
{
    NSLog(@"WLANBackend: linuxSetEnabled %@", enabled ? @"ON" : @"OFF");
...
- (void)linuxDisconnect
{
    ...
    NSLog(@"WLANBackend: linuxDisconnect ssid=%@", ssid);
    if (ssid) {
        int ret = runPrivilegedCommand(_nmcli, @[@"connection", @"down", @"id", ssid]);
        NSLog(@"WLANBackend: linuxDisconnect result=%d", ret);
    }
}
