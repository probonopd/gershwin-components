...
main(int argc, char** argv, char **env_c)
{
  ...
  for (index = 1; index < [args count]; index++)
      ...
      if ([[args objectAtIndex: index] hasPrefix: @"--service="])
	{
      ...
      if ([[args objectAtIndex: index] hasPrefix: @"--lookup="])
	{
	  lookupValue = [[args objectAtIndex: index] substringFromIndex: 9];
	}
...
"  --lookup=XXX   shows which applications open the given filename extension\n"
"                 or mime type (e.g. --lookup=gcode or --lookup=model/stl).\n"
"",
[cachePath cString], [appsPath cString]);
  ...
  /*
   *	Also scan Library/CoreServices/Applications in every domain, where
   *	system components such as the menu bar and window manager live.
   */
  enumerator = [NSSearchPathForDirectoriesInDomains(
  ...
  while ((path = [enumerator nextObject]) != nil)
      ...
      if ([path hasPrefix: @"."] == NO)
	{
	  scanApplications(services,
	    [path stringByAppendingPathComponent: @"CoreServices/Applications"]);
	}
    }

  /*
   *	Also scan the user's Downloads directory, where downloaded AppImage
   *	applications are commonly kept.
   */
  enumerator = [NSSearchPathForDirectoriesInDomains(
    NSDownloadsDirectory, NSUserDomainMask, YES) objectEnumerator];
  while ((path = [enumerator nextObject]) != nil)
    {
      if ([path hasPrefix: @"."] == NO)
	{
  ...
  if (lookupValue != nil)
    {
      printLookupForValue(lookupValue);
      exit(EXIT_SUCCESS);
    }
...
static NSDictionary *
parseDesktopFile(NSString *content)
{
  ...
  for (i = 0; i < [lines count]; i++)
      ...
      if (eq.location == NSNotFound)
	{
      ...
      [result setObject: [line substringFromIndex: eq.location + 1]
...
static NSString *
extensionForMimeType(NSString *mimeType)
{
  static NSDictionary	*special = nil;

  if (special == nil)
    {
      special = [NSDictionary dictionaryWithObjectsAndKeys:
...
static void
printLookupForValue(NSString *value)
{
  NSString	*key = value;
  NSString	*display;
  NSDictionary	*apps;
  ...
  if ([value rangeOfString: @"/"].location != NSNotFound)
    {
      key = extensionForMimeType(value);
      display = [NSString stringWithFormat: @"mime type %@ (extension %@)",
	value, key];
    }
  else
    {
      display = [NSString stringWithFormat: @"extension .%@", value];
    }

  if (key == nil || [key length] == 0)
    {
      GSPrintf(stdout, @"%@: no application found\n", value);
      return;
    }

  apps = [extensionsMap objectForKey: [key lowercaseString]];
  ...
  GSPrintf(stdout, @"%@ can be opened by:\n", display);
  sorted = [[apps allKeys] sortedArrayUsingSelector: @selector(compare:)];
...
addMimeTypeForApplication(NSString *mimeType, NSString *app)
{
  ...
  type = [mimeType stringByTrimmingCharactersInSet:
    [NSCharacterSet whitespaceCharacterSet]];
  if ([type length] == 0)
    {
      return;
  ...
  /* URL scheme handlers are schemes, not file types. */
  if ([type hasPrefix: @"x-scheme-handler/"])
    {
      NSString		*scheme;
      NSMutableDictionary	*d;
      ...
      if ([scheme length] == 0)
	{
      ...
      d = [schemesMap objectForKey: scheme];
      if (d == nil)
	{
	  d = [NSMutableDictionary dictionaryWithCapacity: 1];
	  [schemesMap setObject: d forKey: scheme];
	}
      if ([d objectForKey: app] == nil)
	{
	  [d setObject: [NSDictionary dictionaryWithObject: scheme
				forKey: @"NSURLScheme"]
		forKey: app];
	}
      return;
...

  {
    ...
    if (d == nil)
      {
	d = [NSMutableDictionary dictionaryWithCapacity: 1];
	[extensionsMap setObject: d forKey: ext];
      }
    if ([d objectForKey: app] == nil)
      {
...
scanAppImage(NSMutableDictionary *services, NSString *newPath)
{
  ...
  files = [AppImageReader topLevelFilesInAppImage: newPath maxSize: 1 << 20];
  if (files == nil)
    {
      if (verbose > 0)
	{
	  NSLog(@"bad AppImage - %@", newPath);
	}
      return;
    }
  for (NSString *name in files)
    {
      if ([[name pathExtension] isEqualToString: @"desktop"])
	{
	  desktopName = name;
	  break;
	}
  ...
  if (desktopName == nil)
    {
      if (verbose > 0)
	{
	  NSLog(@"no .desktop file in AppImage - %@", newPath);
	}
      return;
    }

  data = [files objectForKey: desktopName];
  content = [[NSString alloc] initWithData: data encoding: NSUTF8StringEncoding];
  if (content == nil)
    {
      if (verbose > 0)
	{
	  NSLog(@"bad .desktop file in AppImage - %@", newPath);
	}
      return;
    }
  desktop = parseDesktopFile(content);
  RELEASE(content);

  appName = [desktop objectForKey: @"Name"];
  if (appName == nil || [appName length] == 0)
    {
      appName = [[newPath lastPathComponent] stringByDeletingPathExtension];
    }
  if (appName == nil || [appName length] == 0)
    {
      return;
  ...
  /* The launcher resolves an app name to its path by looking up
     "<name>.app"/".debug"/".profile", so the AppImage is registered under the
     display name with a ".app" extension, like an app bundle: the key is the
     app name and the value is the absolute path (here, of the executable
     AppImage file).  The extension map keeps the plain display name so "Open
     With" shows e.g. "OrcaSlicer". */
  {
    NSString	*appKey = [appName stringByAppendingPathExtension: @"app"];

    if ([applicationMap objectForKey: appKey] == nil)
      {
	[applicationMap setObject: newPath forKey: appKey];
      }
    else
      {
	if (verbose > 0)
	  {
	    NSLog(@"duplicate app (%@) - %@", appName, newPath);
	  }
	return;
      }
  }
...
scanApplications(NSMutableDictionary *services, NSString *path)
{
  ...
  for (index = 0; index < [contents count]; index++)
	...
	|| [ext isEqualToString: @"profile"]))
	{
	  ...
	  if ([mgr fileExistsAtPath: newPath isDirectory: &isDir] && isDir)
	      ...
	      if (info)
		{
		  ...
		  if (verbose > 0)
		    {
		      NSLog(@"found application %@ - %@", name, newPath);
		    }
		}
	      else if (verbose > 0)
		{
	...
	&& ([ext isEqualToString: @"AppImage"] || [ext isEqualToString: @"appimage"]))
	{
	  newPath = [path stringByAppendingPathComponent: name];
	  newPath = [newPath stringByStandardizingPath];
	  if ([mgr fileExistsAtPath: newPath isDirectory: &isDir] && !isDir)
	    {
	      scanAppImage(services, newPath);
	    }
	  else if (verbose > 0)
	    {
	      NSLog(@"bad application - %@", newPath);
