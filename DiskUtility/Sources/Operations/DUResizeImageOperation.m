...
@interface DUResizeImageOperation ()
...
@implementation DUResizeImageOperation
...
- (void)execute
{
    ...
    if (![backend respondsToSelector:@selector(resizeImage:options:
                                                        progress:
                                                      completion:)]) {
        [self finishWithError:DUErrorMake(
                                  DUErrorUnsupportedOperation,
                                  NSLocalizedString(
                                      @"This backend cannot resize "
                                      @"images.", nil))];
        return;
    }
    [backend resizeImage:self.object
