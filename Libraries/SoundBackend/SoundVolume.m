...
+ (void)increaseVolume
{
    ...
    NSLog(@"SoundVolume: increaseVolume current=%.2f", vol);
    vol += 0.05f;
...
{
    float vol = [self outputVolume];
    NSLog(@"SoundVolume: decreaseVolume current=%.2f", vol);
    vol -= 0.05f;
...
{
    _ensureBackend();
    NSLog(@"SoundVolume: toggleMute backend=%ld alsaControl=%@ alsaCard=%d", (long)_backend, _alsaControl, _alsaCard);
    if (_backend == VolumeBackendALSA) {
        NSString *output = _runAmixer(@[@"-c", [NSString stringWithFormat:@"%d", _alsaCard],
                                         @"sget", _alsaControl]);
