...
#import "DUNetBSDStorageBackend.h"
...
NSString *const DUNetBSDBackendDetailKey = @"DUBackendDetail";

// UFS labels are capped by the FFS on-disk name field; FAT labels by the
// classic 11-character limit. Truncate before the tools reject them.
static const NSUInteger kMaxUFSLabelLength = 15;
static const NSUInteger kMaxFATLabelLength = 11;

// NetBSD reserves partition letter d for the whole disk; templates and child
// assignments must skip it.
...
@implementation DUNetBSDStorageBackend
...
- (NSArray *)discoverStorageObjects:(NSError **)error
{
    NSMutableArray *objects =
        [[[DUNetBSDDeviceDiscovery new] discoverObjects:error]
            mutableCopy];
...
- (NSArray<DUDiskImage *> *)discoverRegisteredImages
{
    ...
    BOOL qemuImg = [DUNetBSDToolCache haveTool:@"qemu-img"];
    ...
    for (NSString *path in imagePaths) {
        ...
        DUDiskImage *image = [[DUDiskImage alloc]
            initWithIdentifier:[@"netbsd-image-"
    ...
    return @{
        ...
        @"Mount management" :
            ([DUNetBSDToolCache haveTool:@"mount"] &&
             [DUNetBSDToolCache haveTool:@"umount"]) ? @"yes" : @"no",
        @"Partitioning" :
            ([DUNetBSDToolCache haveTool:@"disklabel"]) ? @"partial" : @"no",
        @"Filesystem formatting" :
            ([DUNetBSDToolCache haveTool:@"newfs"] ||
             [DUNetBSDToolCache haveTool:@"newfs_msdos"]) ? @"yes" : @"no",
        @"Filesystem repair" :
            ([DUNetBSDToolCache haveTool:@"fsck_ffs"] ||
             [DUNetBSDToolCache haveTool:@"fsck_msdos"]) ? @"yes" : @"no",
        @"Secure erase" :
            [DUNetBSDToolCache haveTool:@"dd"] ? @"partial" : @"no",
        @"RAID management" : @"no",
        ...
        @"Disk image conversion" :
            [DUNetBSDToolCache haveTool:@"qemu-img"] ? @"yes" : @"no",
        @"Disk image resizing" :
            [DUNetBSDToolCache haveTool:@"qemu-img"] ? @"yes" : @"no",
        @"Optical burning" :
...
- (NSArray<NSDictionary *> *)supportedFormatsForObject:(DUStorageObject *)object
{
    ...
    BOOL ufsPossible = [DUNetBSDToolCache haveTool:@"newfs"];
    BOOL fatPossible = [DUNetBSDToolCache haveTool:@"newfs_msdos"];
...
// with the classic "r" convention ("/dev/wd0a" -> "/dev/rwd0a"). A bare disk
                                ...
                                ufsTakesLabel:(BOOL *)outUFSTakesLabel
{
    ...
    if ([normalized isEqualToString:@"ufs"]) {
        ...
        if (outUFSTakesLabel != NULL) {
            // NetBSD newfs accepts an FFSv2 volume label via -L.
...
- (NSString *)mountedNodeAmong:(NSArray<NSString *> *)nodes
{
    NSDictionary<NSString *, NSDictionary *> *table =
        [DUNetBSDDeviceDiscovery currentMountTable];
            ...
            completion:(void (^)(NSError *error))completion
{
    ...
    NSString *checkerPath = checker == nil
        ? nil
        : [DUNetBSDToolCache pathForTool:checker];
          ...
          completion:(void (^)(NSError *error))completion
{
    ...
    if (gate == nil && wipeWithZeros) {
        ddPath = [DUNetBSDToolCache pathForTool:@"dd"];
        if (ddPath == nil) {
            gate = DUErrorMake(
                DUErrorUnsupportedOperation,
              ...
              completion:(void (^)(NSError *error))completion
{
    ...
    if (gate == nil) {
        disklabelPath = [DUNetBSDToolCache pathForTool:@"disklabel"];
    ...
    if (gate == nil) {
        ...
        currentLabel = probe != nil && probe.exitedNormally &&
            ...
            ? [DUNetBKSDisklabelParser parseDisklabelOutput:
           ...
           completion:(void (^)(NSError *error, NSString *mountPoint))completion
{
    ...
    if (gate == nil) {
        NSDictionary *existing =
            [DUNetBSDDeviceDiscovery currentMountTable][object.backendPath];
        if (existing != nil) {
    ...
    NSString *mountPath = gate == nil
        ? [DUNetBSDToolCache pathForTool:@"mount"]
        : nil;
            ...
            completion:(void (^)(NSError *error))completion
{
    ...
    NSString *umountPath = gate == nil
        ? [DUNetBSDToolCache pathForTool:@"umount"]
        : nil;
    ...
    if (gate == nil) {
        if ([object isKindOfClass:[DUStorageVolume class]] &&
            ...
            target = [DUNetBSDDeviceDiscovery
           ...
           completion:(void (^)(NSError *error))completion
{
    ...
    NSString *ejectPath = gate == nil
        ? [DUNetBSDToolCache pathForTool:@"eject"]
        : nil;
    ...
    [self spawnWork:^{
        ...
        for (NSString *node in [self nodesOfSubtree:object]) {
            NSDictionary *entry =
                [DUNetBSDDeviceDiscovery currentMountTable][node];
            NSString *mountPoint =
                ...
                completion:(void (^)(NSError *error))completion
{
    ...
    NSString *ddPath = gate == nil
        ? [DUNetBSDToolCache pathForTool:@"dd"]
        : nil;
...
- (NSString *)burnToolPath
{
    for (NSString *candidate in @[ @"cdrecord", @"wodim", @"xorriso",
                                    @"growisofs" ]) {
        NSString *path = [DUNetBSDToolCache pathForTool:candidate];
        if (path != nil) {
            return path;
        }
    }
...
- (NSArray<NSDictionary *> *)imageCreationFormats
{
    ...
    if ([DUNetBSDToolCache haveTool:@"gzip"]) {
        [formats addObject:@{
    ...
    if ([DUNetBSDToolCache haveTool:@"qemu-img"]) {
...
// Attaches a raw image file via vnconfig and returns the /dev/vnd0d node, or
// nil on failure. The caller mounts, copies and later detaches it.
- (NSString *)attachImageFile:(NSString *)path
{
    NSString *vnconfig = [DUNetBSDToolCache pathForTool:@"vnconfig"];
    if (vnconfig == nil) {
        return nil;
    }
    NSError *runError = nil;
                     ...
                     completion:(void (^)(NSError *))completion
{
    [self spawnWork:^{
        ...
        if ([format isEqualToString:@"raw"]) {
            NSString *truncate =
                [DUNetBSDToolCache pathForTool:@"truncate"];
            if (truncate == nil) {
                completion(
            ...
            NSString *qemu =
                [DUNetBSDToolCache pathForTool:@"qemu-img"];
            if (qemu == nil) {
                completion([self
                  ...
                  completion:(void (^)(NSError *))completion
{
    ...
    [self spawnWork:^{
        ...
        NSString *truncate = [DUNetBSDToolCache pathForTool:@"truncate"];
        if (truncate == nil) {
        ...
        NSString *newfs =
            [DUNetBSDToolCache pathForTool:@"newfs_msdos"];
        if (newfs == nil) {
            completion([self toolFailure:DUErrorBackendUnavailable
                                  message:NSLocalizedString(
                  ...
                  completion:(void (^)(NSError *, NSString *))completion
{
    [self spawnWork:^{
        ...
        NSError *launchError = nil;
        DUProcessResult *mountResult = [DUProcessRunner
            runExecutable:[DUNetBSDToolCache pathForTool:@"mount"]
               arguments:@[ @"-t", @"msdos", node, mountPoint ]
                   error:&launchError];
        if (![self runSucceeded:mountResult] || launchError != nil) {
            [self detachImageNode:node];
          ...
          completion:(void (^)(NSError *))completion
{
    ...
    NSString *qemuImg = [DUNetBSDToolCache pathForTool:@"qemu-img"];
    if (qemuImg == nil) {
        ...
        completion:(void (^)(NSError *))completion
{
    ...
    NSString *cmp = [DUNetBSDToolCache pathForTool:@"cmp"] ?: @"/usr/bin/cmp";
